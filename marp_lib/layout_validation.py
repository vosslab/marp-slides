...
#============================================
def source_error(source: marp_lib.native_model.SourceLocation | marp_lib.native_model.Slide,
...
#============================================
def unsupported_block(blocks: tuple[marp_lib.native_model.Block, ...]) -> marp_lib.native_model.Block | None:
	...
	for block in blocks:
		if not isinstance(block, (marp_lib.native_model.Heading, marp_lib.native_model.Paragraph,
				marp_lib.native_model.Image, marp_lib.native_model.ListBlock, marp_lib.native_model.Table)):
			return block
	return None


#============================================
def attributed_list_item(item: marp_lib.native_model.ListItem) -> marp_lib.native_model.ListItem | marp_lib.native_model.Block | None:
...
#============================================
def attributed_block(block: marp_lib.native_model.Block) -> marp_lib.native_model.ListItem | marp_lib.native_model.Block | None:
	...
	if isinstance(block, marp_lib.native_model.ListBlock):
		for item in block.items:
			...
			if attributed is not None:
				return attributed
	if isinstance(block, marp_lib.native_model.QuoteBlock):
...
#============================================
def attributed_source(blocks: tuple[marp_lib.native_model.Block, ...]) -> marp_lib.native_model.ListItem | marp_lib.native_model.Block | None:
...
#============================================
def inline_math_location(inlines: tuple[marp_lib.native_model.Inline, ...],
		location: marp_lib.native_model.SourceLocation) -> marp_lib.native_model.SourceLocation | None:
	"""Return the containing source location for unsupported inline math."""
	for inline in inlines:
		if isinstance(inline, marp_lib.native_model.InlineMath):
			return location
		if isinstance(inline, (marp_lib.native_model.Strong, marp_lib.native_model.Emphasis,
				marp_lib.native_model.Link)):
...
#============================================
def list_item_inline_math_location(item: marp_lib.native_model.ListItem) -> marp_lib.native_model.SourceLocation | None:
...
#============================================
def block_inline_math_location(block: marp_lib.native_model.Block) -> marp_lib.native_model.SourceLocation | None:
...
#============================================
def source_inline_math_location(blocks: tuple[marp_lib.native_model.Block, ...]) -> marp_lib.native_model.SourceLocation | None:
...
#============================================
def revealed_list_item(item: marp_lib.native_model.ListItem) -> marp_lib.native_model.ListItem | marp_lib.native_model.ListBlock | None:
...
#============================================
def revealed_block(block: marp_lib.native_model.Block) -> marp_lib.native_model.ListItem | marp_lib.native_model.Block | None:
...
#============================================
def revealed_source(blocks: tuple[marp_lib.native_model.Block, ...]) -> marp_lib.native_model.ListItem | marp_lib.native_model.Block | None:
...
#============================================
def is_implicit_answer_reveal(reveal: marp_lib.native_model.Reveal | None) -> bool:
	"""Recognize the sole whole-popup reveal the Djot parser adds."""
	return reveal == marp_lib.native_model.Reveal(
...
#============================================
def validate_multiple_choice(source: marp_lib.native_model.Slide) -> None:
	...
	choice_list = next((block for block in question.blocks
		if isinstance(block, marp_lib.native_model.ListBlock) and has_visible_items(flatten_list(block))), None)
	...
	if question_images and not isinstance(question.blocks[0], marp_lib.native_model.Image):
	...
	for index, answer_block in enumerate(answer.blocks):
		if not isinstance(answer_block, marp_lib.native_model.Paragraph):
...
#============================================
def validate_table(table: marp_lib.native_model.Table) -> None:
...
#============================================
def validate_table_region(blocks: tuple[marp_lib.native_model.Block, ...],
		location: marp_lib.native_model.SourceLocation, context: str) -> marp_lib.native_model.Table | None:
	"""Return a sole table, or explain why the region has no table destination."""
	tables = [block for block in blocks if isinstance(block, marp_lib.native_model.Table)]
	if not tables:
		return None
	if len(tables) > 1:
	...
	table = tables[0]
	for block in blocks:
		if block is table or isinstance(block, marp_lib.native_model.Heading):
			continue
...
#============================================
def validate_flow_region(blocks: tuple[marp_lib.native_model.Block, ...],
		location: marp_lib.native_model.SourceLocation, context: str) -> None:
	"""Allow ordered text/list and image flow while retaining table's sole-mode rule."""
	headings = tuple(block for block in blocks if isinstance(block, marp_lib.native_model.Heading))
	...
	non_headings = tuple(block for block in blocks if not isinstance(block, marp_lib.native_model.Heading))
	tables = tuple(block for block in non_headings if isinstance(block, marp_lib.native_model.Table))
	if tables:
...
def validate_layout_source(source: marp_lib.native_model.Slide, spec: object) -> None:
	...
	if not spec.allows_root_body:
		offending = next((block.location for block in body.blocks if isinstance(block,
			marp_lib.native_model.Image)), body.location)
