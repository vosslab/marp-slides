...
#============================================
def parse_inlines(path: pathlib.Path, line: int, source: str) -> tuple[marp_lib.native_model.Inline, ...]:
...
def _parse_runs(path: pathlib.Path, line: int, source: str, start: int,
		end: int) -> tuple[marp_lib.native_model.Inline, ...]:
	"""Parse a bounded span, retaining its original offsets for diagnostics."""
	runs: list[marp_lib.native_model.Inline] = []
	text_start = start
	index = start
	while index < end:
		...
		if character == "\\" and index + 1 < end and source[index + 1] == "\n":
			...
			runs.append(marp_lib.native_model.Break())
			index += 2
			text_start = index
			continue
		...
		if character in ("_", "*") and _opens_delimiter(source, index, end):
			...
			if character == "_":
				runs.append(marp_lib.native_model.Emphasis(children))
			else:
				runs.append(marp_lib.native_model.Strong(children))
			index = close + 1
			text_start = index
			continue
		if character == "`":
			...
			runs.append(marp_lib.native_model.InlineCode(value))
			index = close + 1
			text_start = index
			continue
		if character == "[":
			...
			runs.append(marp_lib.native_model.Link(children, url))
			index = url_end + 1
			text_start = index
			continue
		if character == "$":
			...
			runs.append(marp_lib.native_model.InlineMath(value))
			index = close + 1
			text_start = index
			continue
		if character == "~":
...
#============================================
def _append_text(runs: list[marp_lib.native_model.Inline], value: str) -> None:
	"""Append ordinary projected text, coalescing only adjacent text runs."""
	if not value:
		return
	if runs and isinstance(runs[-1], marp_lib.native_model.Text):
		previous = runs.pop()
		...
		projected = marp_lib.djot_grammar.project_text(previous.value + value)
		runs.append(marp_lib.native_model.Text(projected))
	else:
		projected = marp_lib.djot_grammar.project_text(value)
		runs.append(marp_lib.native_model.Text(projected))
...
def _error(path: pathlib.Path, line: int, source: str, index: int,
		message: str) -> marp_lib.djot_errors.DjotParseError:
	"""Create one diagnostic at the physical line containing an inline token."""
	location = marp_lib.native_model.SourceLocation(path, line + source[:index].count("\n"))
	return marp_lib.djot_errors.source_error(location, message)
