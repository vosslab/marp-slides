...
def component_layout(components: list[EmissionComponent]) -> tuple[str, tuple[str, ...], tuple[int, ...]]:
	...
	if coarse_inset_key_layout(components):
		return "two-panels", marp_lib.layouts.LAYOUTS["two-panels"].slot_names, (0, 1)
	if coarse_picture_inset_layout(components):
		...
		return "two-panels", marp_lib.layouts.LAYOUTS["two-panels"].slot_names, (picture_index, body_index) \
	...
	if footer_match is not None:
		...
		return "two-over-one-panels", marp_lib.layouts.LAYOUTS["two-over-one-panels"].slot_names, order
	...
	if explanatory_match is not None:
		...
		return "two-panels", marp_lib.layouts.LAYOUTS["two-panels"].slot_names, order
	...
	name, order = match
	return name, marp_lib.layouts.LAYOUTS[name].slot_names, order
