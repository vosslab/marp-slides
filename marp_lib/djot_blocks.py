...
class _BlockSpan:
	...
	block: marp_lib.native_model.Block
	first_line: int
	after_line: int
...
class ParsedBlocks:
	...
	@property
	def blocks(self) -> tuple[marp_lib.native_model.Block, ...]:
		"""Return parsed blocks in source reading order."""
		blocks = tuple(span.block for span in self.spans)
		return blocks

	def block_before(self, line: int) -> marp_lib.native_model.Block | None:
	...
	def block_after(self, line: int) -> marp_lib.native_model.Block | None:
...
#============================================
def parse_attribute_tokens(path: pathlib.Path, line: int, body: str) -> tuple[marp_lib.native_model.Attribute, ...]:
	"""Parse the compact one-line Djot attribute form without a new dependency."""
	attributes: list[marp_lib.native_model.Attribute] = []
	index = 0
	while index < len(body):
		...
		if name.startswith("#"):
			...
			attributes.append(marp_lib.native_model.Attribute("id", name[1:]))
			continue
		if name.startswith("."):
			if len(name) == 1:
				fail(path, line, "Djot class attributes require a value")
			attributes.append(marp_lib.native_model.Attribute("class", name[1:]))
			continue
		if index == len(body) or body[index].isspace():
			attributes.append(marp_lib.native_model.Attribute(name, None))
			continue
		index += 1
		if index == len(body):
		...
		if body[index] in "\"'":
			...
			while index < len(body) and not body[index].isspace():
				index += 1
			value = body[value_start:index]
		attributes.append(marp_lib.native_model.Attribute(name, value))
	result = tuple(attributes)
	return result


#============================================
def parse_attributes(path: pathlib.Path, line: int, source: str) -> tuple[marp_lib.native_model.Attribute, ...] | None:
...
#============================================
def add_attributes(block: marp_lib.native_model.Block,
		attributes: tuple[marp_lib.native_model.Attribute, ...]) -> marp_lib.native_model.Block:
...
#============================================
def add_item_attributes(item: marp_lib.native_model.ListItem,
		attributes: tuple[marp_lib.native_model.Attribute, ...]) -> marp_lib.native_model.ListItem:
...
def replace_last_block(parsed: ParsedBlocks,
		transform: Callable[[marp_lib.native_model.Block], marp_lib.native_model.Block]) -> ParsedBlocks:
...
def replace_last_list_item(parsed: ParsedBlocks,
		transform: Callable[[marp_lib.native_model.ListItem], marp_lib.native_model.ListItem]) -> ParsedBlocks:
	"""Apply a deck-assembly action to the deepest preceding list item."""
	if not parsed.spans or not isinstance(parsed.spans[-1].block, marp_lib.native_model.ListBlock):
...
#============================================
def replace_deepest_last_list_item(list_block: marp_lib.native_model.ListBlock,
		transform: Callable[[marp_lib.native_model.ListItem], marp_lib.native_model.ListItem]) -> marp_lib.native_model.ListBlock:
...
#============================================
def parse_image(path: pathlib.Path, line: int, source: str) -> marp_lib.native_model.Image | None:
	...
	image = marp_lib.native_model.Image(location(path, line), match.group("alt"), image_source,
		match.group("title"))
	return image
...
def parse_fence(path: pathlib.Path, lines: list[str], index: int,
		base_line: int) -> tuple[marp_lib.native_model.CodeBlock, int]:
	...
	block = marp_lib.native_model.CodeBlock(location(path, base_line + index), value, language)
	return block, end + 1


#============================================
def parse_display_math(path: pathlib.Path, lines: list[str], index: int,
		base_line: int) -> tuple[marp_lib.native_model.DisplayMath, int]:
	"""Parse one single-line or fenced Djot display-mathematics expression."""
	source = lines[index].strip()
	if source != "$$":
		value = source[2:-2].strip()
		block = marp_lib.native_model.DisplayMath(location(path, base_line + index), value)
...
def parse_list(path: pathlib.Path, lines: list[str], index: int, base_line: int,
		indent: int | None = None) -> tuple[marp_lib.native_model.ListBlock, int]:
	...
	block_attributes: tuple[marp_lib.native_model.Attribute, ...] = ()
	...
	while index < len(lines):
		...
		while index < len(lines):
			...
			nested_boundary = False
		text = "\n".join(text_lines)
		inlines = marp_lib.djot_inline.parse_inlines(path, line, text)
		items.append(marp_lib.native_model.ListItem(location(path, line), inlines, tuple(children),
			attributes=()))
	if not items:
		fail(path, base_line + index, "Djot lists require an item")
	block = marp_lib.native_model.ListBlock(location(path, base_line + first_index), ordered, start, tuple(items),
...
def parse_table(path: pathlib.Path, lines: list[str], index: int,
		base_line: int) -> tuple[marp_lib.native_model.Table, int]:
	"""Parse a Djot table, preserving only metadata expressible in the IR."""
	first_line = base_line + index
	rows: list[tuple[tuple[marp_lib.native_model.Inline, ...], ...]] = []
	width: int | None = None
	...
	while index < len(lines) and lines[index].lstrip(" ").startswith("|"):
		...
		if alignment is not None:
			...
			continue
		parsed_cells = tuple(
			marp_lib.djot_inline.parse_inlines(path, base_line + index, cell)
			for cell in cells
		)
		rows.append(parsed_cells)
		index += 1
	if separator_index in (None, 0):
		headers: tuple[tuple[marp_lib.native_model.Inline, ...], ...] = ()
		body_rows = tuple(rows)
	else:
		headers = rows[0]
		body_rows = tuple(rows[1:])
	block = marp_lib.native_model.Table(location(path, first_line), headers, body_rows)
	return block, index


#============================================
def parse_quote(path: pathlib.Path, lines: list[str], index: int,
		base_line: int) -> tuple[marp_lib.native_model.QuoteBlock, int]:
	...
	block = marp_lib.native_model.QuoteBlock(location(path, first_line), nested.blocks)
	return block, index
...
def parse_blocks(path: pathlib.Path, source: str, base_line: int = 1) -> ParsedBlocks:
	...
	index = 0
	pending_attributes: tuple[marp_lib.native_model.Attribute, ...] = ()
	pending_line: int | None = None
	while index < len(lines):
		if not lines[index].strip():
			index += 1
		...
		if fence is not None:
			...
			if heading is not None:
				block = marp_lib.native_model.Heading(location(path, line), len(heading.group("marks")),
					marp_lib.djot_inline.parse_inlines(path, line, heading.group("text")))
				next_index = index + 1
			else:
				...
				if image is not None:
					...
					block = marp_lib.native_model.Paragraph(location(path, line),
						marp_lib.djot_inline.parse_inlines(path, line, paragraph))
					next_index = index
