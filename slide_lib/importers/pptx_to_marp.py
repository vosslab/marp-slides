...
def render_slide(slide: SlideData, slide_width: int, is_first: bool) -> tuple[list[str], str]:
	...
	if not images:
		...
		return ["<!-- _class: one-panel -->", *heading_lines, "", *texts], "one-panel"
	if len(images) == 1 and not texts:
		return ["<!-- _class: one-panel -->", *heading_lines, "", image_markdown(images[0])], "one-panel"
	if len(images) == 1:
		image = images[0]
		center = (image.left + image.width / 2) / slide_width
		if center >= 0.55:
			return [
				"<!-- _class: two-panels -->", *heading_lines, "",
...
def run_import(input_file: pathlib.Path, output_file: pathlib.Path | None = None) -> None:
	"""Run one PPTX-to-Marp conversion."""
	output_path = input_file.with_suffix(".md") if output_file is None else output_file
