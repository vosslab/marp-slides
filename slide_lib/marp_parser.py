...
FONT_SIZE_CLASSES = frozenset(f"font-size-{preset.value}" for preset in slide_lib.native_model.FontSizePreset)
...
def location(path: pathlib.Path, line: int) -> slide_lib.native_model.SourceLocation:
	...
	return slide_lib.native_model.SourceLocation(path, line)
...
def remove_comments(path: pathlib.Path, raw_slide: str, base_line: int,
		default_paginate: bool) -> tuple[str, str, slide_lib.native_model.TitleSizeOverride | None, bool, tuple[str, ...]]:
	"""Extract standalone directives and notes while retaining source line offsets."""
	layout_class: str | None = None
	title_size_override: slide_lib.native_model.TitleSizeOverride | None = None
	...
	def replace(match: re.Match[str]) -> str:
		...
		if comment.startswith("_class:"):
			...
			if size_tokens:
				title_size_override = slide_lib.native_model.TitleSizeOverride(location(path, line),
					slide_lib.native_model.FontSizePreset(int(size_tokens[0].removeprefix("font-size-"))))
...
#============================================
def parse_inlines(path: pathlib.Path, line: int, children: list[Token]) -> tuple[slide_lib.native_model.Inline, ...]:
	...
	while index < len(children):
		...
		if token.type == "text":
			result.append(slide_lib.native_model.Text(token.content))
		elif token.type in ("softbreak", "hardbreak"):
			result.append(slide_lib.native_model.Break())
		elif token.type == "code_inline":
			result.append(slide_lib.native_model.InlineCode(token.content))
			...
			if token.type == "strong_open":
				result.append(slide_lib.native_model.Strong(inner))
			elif token.type == "em_open":
				result.append(slide_lib.native_model.Emphasis(inner))
			else:
				...
				result.append(slide_lib.native_model.Link(inner, url))
			index = end - 1
...
#============================================
def parse_image(path: pathlib.Path, line: int, token: Token) -> slide_lib.native_model.Image:
	...
	return slide_lib.native_model.Image(location(path, line), alt_text, source, token.attrGet("title"))


#============================================
def parse_list(path: pathlib.Path, tokens: list[Token], index: int, base_line: int) -> tuple[slide_lib.native_model.ListBlock, int]:
	...
	while tokens[index].type != opening.type.replace("_open", "_close"):
		...
		index += 1
		inlines: tuple[slide_lib.native_model.Inline, ...] | None = None
		...
		items.append(slide_lib.native_model.ListItem(location(path, item_line), inlines, tuple(children)))
		index += 1
	index += 1
	return slide_lib.native_model.ListBlock(location(path, token_line(opening, base_line)), ordered, start,
		tuple(items)), index


#============================================
def parse_blocks(path: pathlib.Path, tokens: list[Token], base_line: int,
		allow_cells: bool) -> tuple[tuple[slide_lib.native_model.Block, ...], tuple[slide_lib.native_model.Cell, ...]]:
	"""Convert supported block tokens and preserve top-level quote cells."""
	blocks: list[slide_lib.native_model.Block] = []
	cells: list[slide_lib.native_model.Cell] = []
	index = 0
	while index < len(tokens):
		...
		if token.type == "heading_open":
			inline = tokens[index + 1]
			blocks.append(slide_lib.native_model.Heading(location(path, line), int(token.tag[1:]),
			...
			if images:
				...
				blocks.append(slide_lib.native_model.Paragraph(location(path, line),
			...
			cells.append(slide_lib.native_model.Cell(location(path, line), cell_blocks))
			index = end
...
#============================================
def normalize_layout_cells(source: slide_lib.native_model.Slide,
		blocks: tuple[slide_lib.native_model.Block, ...],
		cells: tuple[slide_lib.native_model.Cell, ...]) -> tuple[tuple[slide_lib.native_model.Block, ...],
		tuple[slide_lib.native_model.Cell, ...]]:
	"""Bind legacy Marp regions to the layout's named native cell contract."""
	spec = layouts.LAYOUTS[source.layout_class]
	global_blocks = tuple(block for block in blocks if isinstance(block, slide_lib.native_model.Heading))
	root_content = tuple(block for block in blocks if not isinstance(block, slide_lib.native_model.Heading))
	if cells:
		named_cells = tuple(slide_lib.native_model.Cell(cell.location, cell.blocks,
	...
	if root_content and spec.slot_names and (spec.allows_root_body or spec.name == "gallery"):
		return global_blocks, (slide_lib.native_model.Cell(root_content[0].location, root_content,
			spec.slot_names[0]),)
...
def parse_deck(input_path: pathlib.Path) -> slide_lib.native_model.Deck:
	...
	slides: list[slide_lib.native_model.Slide] = []
	for raw_slide, slide_line in split_slides(path, body, body_line):
		...
		provisional = slide_lib.native_model.Slide(location(path, slide_line), layout_class,
		...
		if title_size_override is not None:
			...
			titles = [block for block in blocks if isinstance(block, slide_lib.native_model.Heading) and block.level == 1]
			if len(titles) != 1:
				offending = titles[1].location if len(titles) > 1 else next((block.location for block in blocks
					if isinstance(block, slide_lib.native_model.Heading)), title_size_override.location)
				raise error(path, offending.line, "font-size modifier requires exactly one top-level H1 title")
		slides.append(slide_lib.native_model.Slide(location(path, slide_line), layout_class, title_size_override, paginate,
			notes, blocks, cells))
	if not slides:
	...
	return slide_lib.native_model.Deck(path, path.parent, repo_root, str(front_matter.get("title", "")),
