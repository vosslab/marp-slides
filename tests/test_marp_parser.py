...
def test_parses_typed_slide_semantics_and_source_lines(tmp_path: pathlib.Path) -> None:
	...
	deck = parse(tmp_path, "<!-- _class: two-panels -->\n<!-- _paginate: false -->\n"
...
#============================================
def test_normalizes_root_body_and_gallery_content_into_named_cells(tmp_path: pathlib.Path) -> None:
	"""Legacy root Markdown projects into the named layout-cell IR boundary."""
...
@pytest.mark.parametrize(("body", "message"), [
	("<!-- _class: one-panel centered-text -->\n# Title\n", "exactly one canonical layout"),
